...
export function UploadIsland({ subjects }: { subjects: SubjectSummary[] }) {
  ...
  async function upload() {
    ...
    try {
      ...
      setSelectedFiles([]); setMessage("Material processed and ready."); router.refresh();
    } catch (error) { setMessage(error instanceof Error ? error.message : "Upload failed."); }
    finally { setPending(false); }
  }
  return <div className={styles.islandContent}><CustomSelect label="Save to subject" value={String(uploadSubject)} options={options} onChange={(value) => setUploadSubject(Number(value))} /><div className={styles.fileField}><span className={styles.controlLabel}>Choose files</span><input ref={fileRef} className={styles.hiddenFile} id="material-files" type="file" multiple accept=".pdf,.docx,.txt" onChange={(event) => setSelectedFiles(event.target.files ? Array.from(event.target.files) : [])} /><label className={styles.dropzone} htmlFor="material-files"><span className={styles.dropIcon}><Files size={29} weight="duotone" /></span><span><strong>{selectedFiles.length ? `${selectedFiles.length} ${selectedFiles.length === 1 ? "file" : "files"} selected` : "Choose readings to upload"}</strong><small>{selectedFiles.length ? selectedFiles.map((file) => file.name).join(", ") : "Browse from this device"}</small></span><span className={styles.browseButton}>Browse</span></label></div><button className={styles.uploadButton} disabled={pending || !uploadSubject || !selectedFiles.length} onClick={upload}>{pending ? "Processing material…" : "Upload and process"}<ArrowRight size={18} weight="bold" /></button>{message && <div className={`${styles.message} ${/failed|could not|cannot/i.test(message) ? styles.messageError : ""}`} role="status"><CheckCircle size={18} weight="fill" />{message}</div>}</div>;
}
