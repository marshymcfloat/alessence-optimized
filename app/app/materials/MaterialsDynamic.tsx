...
export async function MaterialsHeaderStats() {
  const [materials, subjects] = await Promise.all([materialsPageMaterials(), materialsPageSubjects()]);
...
export async function MaterialLibrary() {
  const [materials, subjects] = await Promise.all([materialsPageMaterials(), materialsPageSubjects()]);
  const revision = `${materials.map((item) => `${item.id}:${item.name}:${item.subject?.title}`).join("|")}::${subjects.map((item) => `${item.id}:${item.title}`).join("|")}`;
